...
「HLS 配置」里的「存货保险线」可调，0=关）：低于它 → 预取并发钳到 3；低于它的 40% → 钳到 2。
补够了自动放开，交回原来的闭环爬满。

- **为什么少开更快**：决定「现在能不能播下去」的只有紧邻播放头那一两片，而同 host 只有 6 个连接槽。
  多开的每一条都在下更远的分片，却要跟那一片抢连接和带宽——**越缺越多开，最需要的那片越晚到**。
  实测截图：源站判「差」档（`concurrencyFloor: 6`）、标着「可并行」，卡到已缓冲 0.3s 仍在跑 6 线程，
  而聚合速度 16.8 Mbps 是码率 2.1 Mbps 的八倍——**带宽压根不是瓶颈，摊薄才是**
- **必须按倍速换算**：3x 下缓存 6 秒只够播 2 秒。与起播门槛用同一把尺子
- 它**排在 `floorConn` 与「暂停→顶格并发」之后**生效，两者都要压住：起播那一刻恰好是
  「还没 play() 所以 `paused` 为真」+「缓存为 0」同时成立，不压就是满并发去抢第一片
...
#### 存货**快够了**也要少开线程：并发上限 = 缺口装得下几片（`headroomConnCap`）

上一条管「不够」，这一条管「够了」，目标都是**让缓存收敛到「预加载时长」而不是冲过它**：
同时在下的分片数不超过 `(预加载时长 − 已有缓存) ÷ 分片时长`。缺口 20s、每片 10s → 2 条就填满了，
开 6 条里 4 条在下超出目标的部分，下完还要被停取判定或 LRU 淘汰，纯属白占连接和带宽。

- **踩过**：预加载时长 100s、已有 98s，缺口不到一片，线程数却钉在 6/12。原因是
  `floorConn()`（「差」档 `concurrencyFloor: 6`）和「暂停 → 顶格并发」把闭环那句
  「接近目标就 −1」整个压回去了。所以这条上限**必须排在它们之后**（和存货保险线同一位置）
- 顺带把并发做成一条**随缺口线性收敛的斜坡**（缺口 120s→12 条、60s→6、30s→3、10s→1）：
  缓存稳稳停在预加载时长附近，缺口一张开线程立刻跟着张开。**没有「充足」阈值可调**——
  目标就是用户填的「预加载时长」，两个数字对齐比再加一个输入框好
- **用视频秒数算，不除倍速**：缺口和分片时长都在视频时间轴上，除以倍速两头都缩、白折腾。
  抗卡那两档才是墙钟（「够播几秒」），两者管相反方向，各用各的尺子
- 分片时长取清单的 `targetduration`（它就是「最长的一片」，算上限正合适），拿不到退回真实
  `frag.duration`，都没有时兜底 10s——那只发生在冷启动一两拍，那时缺口远大于一片，这条压根不咬人
