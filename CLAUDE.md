...
### 进度条缩略图（`useVideoThumbnails.ts`）

两路取帧，**先白捡再说别的**：① 正播着的画面每跨一个 10 秒桶就顺手截一帧（零网络零解码器，
往回拖时全是现成的）；② 悬浮到没截过的位置，才起隐藏解码器解那一片。

- **清单一次都不取**：分片表从主播放的 hls 实例上现读（`getHls().levels[..].details.fragments`）——
  那份清单主播放早就解析好了，再取一遍既慢又可能撞防盗链
- **分片先吃主播放的缓存**（`getSegBuf`，零网络）；缓存里没有的（拖到片子后半段）**等指针停住
  320ms 才下**（`SETTLE_MS`）。这道停留判据是「扫过进度条不该下载」与「拖到后面也要有图」
  之间唯一的分界：扫一趟点名几十个桶，逐个下会把连接全占死；停住不动 = 他真的想看那儿。
  下回来的片**单独存 3 片，绝不塞进主预取缓存**（那份容量是按「够播几秒」算的，塞进去会把
  播放头前面真要用的挤出去）。在途只留一发，指针一挪就 abort
- **要下也只读开头那 100KB 就 `abort`**：一张 240px 的图要的就是一帧，而一片 500KB~2MB
  （实测 753KB / 614ms）。分片都以关键帧开头，这一段通常够解出第一帧，截断处那半个帧
  transmuxer 自己会丢。**用 `body.getReader()` 自己数着字节读，不用 `Range` 头**：
  Range 会让跨域直连触发 CORS 预检（同「直连探测绝不能加 Range」那条），逼得这一发只能走代理，
  而**走代理比直连慢一大截**（实测 803KB 代理 9.76s，直连一秒出头）——省那点流量远不抵多绕一圈。
  也别用 `res.arrayBuffer()`：那是「读完为止」，实测撞到过 1.2MB / 7.8s
- **连接方式照搬主播放**（`getProxyUrl` + `no-referrer`）：直连的源就直连，该注入防盗链头的才走代理。
  解不出就**按阶梯加量**（100KB → 8× → 24×），**绝不直接退回整片**；每档的解码等待也要短
  （`LOAD_TRY_MS` 2.5s，三档都等满 `SEEK_TIMEOUT_MS` 的话一张图要等小二十秒）
- **喂给解码器的是一份自己拼的 `blob:` 迷你清单**（只含目标那一片 + 必要时的 `EXT-X-MAP`）：
  这样解码器不会自作主张往后预读，分片 URL 直接用主播放那份、与分片缓存**同键**。
  时间轴从 0 开始 → seek 的是**片内偏移**（`at - frag.start`）。blob 用完要 `revokeObjectURL`
- **踩过**：miss 时交给 hls.js 默认 XHR loader 自己下，分片一律 **403**（Network 里写着
  `strict-origin-when-cross-origin`）——XHR 发得出 Referer 而这些 CDN 认防盗链，且没过连接策略。
  自己下那一发必须 `getProxyUrl` + `referrerPolicy: 'no-referrer'`，与主播放**完全同形**
  （同形还顺带吃到浏览器对主播放那些分片的 HTTP 缓存）
- **加密片（`decryptdata`）直接放弃**：拼一份缺 `EXT-X-KEY` 的清单会解出一堆噪声帧。宁可没图
- **等 `FRAG_BUFFERED` 才 seek**，光有 `loadedmetadata` 时数据还没进 buffer，seek 过去抓的是黑帧。
  抓帧本身还要等 `requestVideoFrameCallback`——`seeked` 早于新帧呈现，当场抓会抓回上一帧
- **seek 目标必须钳进 `buffered`**：截断片只解得出开头一两秒，而桶中点往往落在片内第 5 秒，
  那里没有数据 → **`seeked` 永远不来**，只能干等到超时，表现是「片下回来了、请求 200，就是不出图」
  （踩过）。桶本来就是 10 秒粒度，退到已解出的那一段里抓，差几秒无所谓。
  另外**目标与当前位置相同时浏览器不派发 `seeked`**（截断片只剩开头、起播位置又恰好是 0 时必然撞上），
  那时直接抓当前帧
...
### 媒体库与换源（左侧悬浮抽屉 `SideDock.vue`）

播放页左边缘一竖条悬浮按钮 → 抽屉两页：**媒体库**（`LibraryPanel` = 播放历史 + 收藏影片，行长得一样，
共用 `LibraryRow`）与**换源**（`LinePicker`，线路表来自最近一次解析）。**不放进页面下方那串折叠卡**：
