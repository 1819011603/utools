...
2. **残影/黑屏（音频与 `currentTime` 正常）→ 不是我们的问题**：浏览器把视频画在独立硬件 overlay 平面，
   ...
   **别用 `display:none`**——那会让 `<video>` 卸掉解码器再重建，真的会黑一下。
   里面那次 `void offsetHeight`（强制同步布局）不能省：只设样式再在下一帧撤销，浏览器可以把两次改动
   合并成「没变过」，整个函数就成了空操作。
...
**解锁键在锁定态下必须无条件渲染**（任何屏幕尺寸）：锁上之后手势层全部 return、控制栏恒隐、快捷键也停，
它是画面上唯一的出口。原来跟锁定键共用「窄屏小窗不出」的条件（那条是给未锁定态的防误触说的），
于是「全屏锁定 → 来电/系统手势退出全屏 → 变窄屏小窗」之后整个播放器点什么都没反应，**只能刷新页面**（踩过）。
中央播放键同理要在锁定态下不渲染：它带 `data-no-gesture` + `@click`，锁了还能暂停，还正好压在解锁键上方。
