...
- **`if (!data.fatal) return` 是这一幕的元凶**（踩过）：`bufferStalledError` 是**非致命、每秒复发**的，
  而它恰恰是唯一的线索——hls.js 卡在一个比 `maxBufferHole`(0.5s) 更大的空洞前，自己 nudge 0.2s 跨不过去，
  就一直报给我们听，而我们一进来就 return 掉了。非致命错误**不能一概丢**
- 两级出路，都要求「播放头处 MSE 为空」且「手上确实有货」：洞 ≤3s → 直接跳到洞后面那段起点
  （等于临时放大 `maxBufferHole`，丢几秒画面比冻住强）；前方压根没有缓冲段 →
  `hls.startLoad(currentTime)` 从播放头重拉（要的几片多半就在预取缓存里，`fLoader` 同步喂回去）
