...
- **队列是模块级单例**（`download/downloadQueue.ts`）：抽屉关掉、切集、离开这一页都得接着跑。
  **串行下一集一集**（并行两集等于把那 6 条连接的账算两遍）。运行时依赖（取址/缓存/让路判据）离开播放页
  **不清空**，那些闭包读到的 ref 仍持有值。有任务在飞时挂 `beforeunload`（Blob 和文件句柄都活不过刷新）
- **绝不跟正在播的那一集抢**：没在播 6 条、正在播 3 条、缓冲不健康 2 条、`panic` 整个停手（勾了「全速下载」才无视）。
  下载器**不参与**九级并发决策、也不往带宽账本里写数（污染分档采样会连累好几级）。
  并发是动态的 → **固定开满 6 条 worker、每条自己看该不该干活**，别按当下并发数重开（会丢掉在途请求）。
  **「片都发完了」这个 return 必须排在所有等待之前**：否则被收窄掉的那几条 worker 在让路循环里一直转，
  `Promise.all` 永不 resolve → **分片全下完了、任务却停在 590/590 不落定**（踩过）
- **并发拉但顺序写**，且乱序窗口只许超前 `并发数 + 2` 片 —— 内存占用因此恒等于「并发数 × 分片大小」。
  **写入必须串成一条链**：几条 worker 同时 flush 会让 `FileSystemWritableFileStream` 抛「locked」、
  Blob 那边则把片序写乱（能播前几分钟、之后花屏）。写失败记进 `failure`，别让链子保持 rejected
  （后面每次 flush 立刻拒，会被误记成「分片失败」）
- **落盘两条路**（`download/fileSink.ts`）：桌面 Chrome/Edge 用 `showDirectoryPicker` +
  `createWritable` 流式写盘，**目录只请一次授权**（勾 10 集要点 10 次保存框的话，后 9 次都发生在用户
  早已切走的时候，压根弹不出来）；其余环境（Safari / 全部手机浏览器）退回 Blob，**上限 3GB**，
  且下到 3 片就按均值**预估**总量提前劝退 —— 等它下到 80% 再崩是最糟的结果。
  失败/取消一律 `abort()` 删掉写了一半的文件（留着就是「下过了但播不了」）
- **单片失败最多跳 3 片并如实报出来**（跳掉的那几秒画面会直接跨过去）；为一片把二十分钟的下载判死太狠，
  但默默跳更糟。**离网期间一律不计失败次数**（同「每一个失败额度都必须是连续失败」那条）
- 取址走 `peekLazyUrl`（静默 + 给前台让路 + 更短死线），**绝不能用前台那版 `resolveWithUi`**
  ——它会在正播着的这一集上盖出「正在获取播放地址」的转圈遮罩
- 通道顺序 `直连 → 代理带头 → 代理 noref`，**成过的那条就粘住**（一集几百片，每片从头试等于把失败乘以片数）；
  防盗链取 `originHint`/`refererHint`（**候选值**，整条线路共用）而不是生效值 —— 后者是「正在播的这一集」
  探出来的结论，下载跑几分钟、中途还会切集，读它就会漂
- **音视频分轨的线路直接拒掉**并说明换线路：两条流拼不到一个文件里，下个只有画面没声音的文件
  比明说不支持糟得多
- **文件名是「集数在前、剧名在后」**（`第02集-现在不是出轨的问题.mp4`）：站点给的集名多半是孤零零一个
  `2`，所以纯数字要补成「第N集」（同 `currentVideoName` 的规矩），并**补零到跟总集数一样宽**
  ——不补的话文件管理器按字典序排成 第1集、第10集、第11集、…、第2集，而集数放前面本来就是为了顺着排。
  不是纯数字的（「上」「第一季01」）原样保留。任务行上只显示集数，剧名只进文件名（文件要离开这一页）
- UI：顶栏收藏**左边**一颗（最右那格留给收藏，肌肉记忆不动），有任务在飞时叠一枚计数徽标；
