...
- 看片当下才改的（自动全屏 / 自动最佳倍速 / 超快倍速 / 跳过片头片尾）→ 控制栏的齿轮菜单 `SettingsMenu`，手不用离开画面。
...
##### 「缓冲几百秒却冻死在某个固定时间点」= 那一片的音视轨不对齐（`maxBufferHole`）

**根因不在我们这边，但兜不住就是彻底播不了**。实测甄嬛传第 67 集 33:07（把分片抠出来 ffprobe 的结论）：

| 清单索引 | 视轨 | 音轨 |
| --- | --- | --- |
| 330 | 2035.525~2041.525 | 2035.525~2041.525 ✓ |
| **331** | 2041.525~2047.525 | **2042.165**~2047.413 ← **开头缺 0.64s** |
| 332 | 2047.525~2053.525 | 2047.413~2053.357 ✓ |

`video.buffered` 是**音轨与视轨的交集**，所以音频那 0.64s 的洞就是播放的硬墙。而致命的一环是
**hls.js 按视轨记账**（它解出的 pts(v) 是完整的 6 秒）→ `fragmentTracker` 认定这片已缓冲 →
`_doTickIdle` 每拍都算出「下一片已经有了」→ **状态停在 IDLE，永远不再请求任何分片**。
这是死锁不是慢：预取缓存里躺着几百秒毫无用处，因为它压根不来要。

- **真正的量级比 ffprobe 看着更糟**：hls.js 那边解出来的音轨只有 **1.3s**（现场日志
  `pts(v)=1986.2~1992.2` 对 `pts(a)=1986.1~1987.4`），也就是这一片有 **4.8 秒没有音频**。
  所以 `maxBufferHole` 调多大都跨不过去（顺手从 0.5 提到 1s，能兜住 0.6~0.9s 那一档常见错位，
  但**不是这一幕的解**）。**唯一的出路是整片放弃**：清掉这一片 + 把播放头挪到下一片
- **清缓冲的范围只给这一片，绝不能 `endOffset: Infinity`**（踩过）：清到无穷会把播放头前方
  所有好数据一起扔掉，日志里就是 `MSE区间 (全空)`——3x 倍速下要从零重新攒够起播门槛，
  画面上跟没恢复毫无区别，还白扔一堆已经下好的分片
- **hls.js 本来有补静音帧的能力**（`remuxAudio` 里 `Injecting N audio frames … due to X ms gap`），
  但它只在这一片与上一片**连续**时才做。**从进度记忆恢复、正好落在这种片里**就不连续，
  于是补静音那条路整条跳过——所以症状是「续看某一集必冻在同一个时间点」，而不是随机偶发
- 排查这类问题**必须把分片抠出来 ffprobe**，光看 hls.js 的读数只会看到「解出来了却没进去」。
  抠的时候注意两件事：① 这些 CDN（实测小红书 `sns-open-qc.xhscdn.com`）把分片**伪装成 PNG**，
  `content-type: image/png` 且真有 PNG 文件头，TS 同步字节在偏移 112 才开始
  （hls.js 的 TS 探测会自己跳过前缀，所以能播）；② `ffprobe` 要喂剥掉前缀的那份

##### 「缓冲 305s 却一直转圈」= MSE 在播放头处有个洞（`engine/stallRecovery.ts`）

三个读数放一起就能定位，**别被 0 线程 / 0 KB/s 带偏**（那两个是正常的：缓存已到「预加载时长」，
预取按设计停了，全命中缓存自然没有网速）：徽标那个「缓冲 305.7s」是**有效可播**
（MSE + 预取缓存），说明数据在手上；真正的问题只有一个 —— **MSE 在播放头那一点是空的**。

- **真凶是 `fLoader` 的同步回调**（踩过，跟 pLoader 那个坑一模一样）：命中预取缓存时原来直接在
  `load()` 里同步 `onSuccess`，而 hls.js 是**在 `load()` 返回之后**才把这一片记成「在加载中」的
  ——同步回调等于在它记账之前交货，那一片的结果被当成无主的丢掉，而它永远停在「还在等这一片」上。
  **缓存越满越容易发生**（越满越多片走同步路径），所以症状是「缓冲 300s 反而冻住」。
  改成 `setTimeout(0)` 延到下一个宏任务（代价几乎为 0）。
  判据很清楚：连 `startLoad(currentTime)` 都救不回来、播放头一秒都不动
  （实测日志 `startLoad(1987.3)` 连打 10 次原地不动）——那说明不是「差数据」，是**交货没人收**
- **`if (!data.fatal) return` 让它整整瞒了一层**（踩过）：`bufferStalledError` 是**非致命、每秒复发**的，
  而它恰恰是唯一的线索——hls.js 卡在一个比 `maxBufferHole`(0.5s) 更大的空洞前，自己 nudge 0.2s 跨不过去，
  就一直报给我们听，而我们一进来就 return 掉了。非致命错误**不能一概丢**
- 自救按**阶梯**升级，每级只用一次，播放头一动就整份归零（**同一招重复十次无效就该换招**——
  日志里那 10 次一模一样的 `startLoad` 本身就是信息）：洞 ≤3s → 跳到洞后面那段起点（不占阶梯，
  每次都先试）；① `startLoad(currentTime)` 重排一次；② 微跳 0.5s 绕过被标成 `gap` 的那一片
  （标了 gap 的片 hls.js 不会再取，只能绕）；③ `recoverMediaError()` 重建 MediaSource
  （append 通道本身坏了时唯一的出路，代价是黑一下）；④ 全过不去就**停手并写 `errorMessage`**
  ——继续打转只会刷满控制台，用户需要的是「换条线路试试」那句话
- **播放头处 MSE 有货就一律不动手**：那是解码/GPU/倍速侧的卡（判读看统计面板的掉帧），乱跳会打断正常播放。
  另外**必须有冷却**（2s）：hls.js 自己的 nudge 每秒都在动 `currentTime`，两套恢复机制互抢方向盘的表现是画面反复小跳
- 心跳里还要**采样播放头兜底**（冻住 ≥2s 就自救）：`bufferStalledError` 并非每次冻结都发，
...
- **探测结论不跨页复用**（`video-probe-cache` 与 `probeStore.saveProbe/loadProbe` 已整块删除）。
  曾经按完整 URL 落一份到 localStorage（TTL 90s、8 条、取用即删）：播放器开的是**新标签页**，
  `warmProbes` 那个 Map 跨不过去，于是解析页「可达性检测」刚测完的同一条地址，播放器起播时又要整轮重测。
  现在一律当场实测，代价就是那一轮（自带两级超时，慢源一两秒）。
  留给后来人的两条：① 这份缓存持久化时必须剥掉 `manifestText`——长剧清单动辄上百 KB，
  几条就能撑爆配额，而写失败是**静默的**，到时候整个缓存都不工作，比不缓存更难查；
  ② 解析页那轮探测（`ReachCheck.vue`）现在只服务它自己那块 UI，别再往外存
...
#### 每一个「失败额度」都必须是「连续失败」，而且断网期间一律不计数

四条额度原来各自都能被**几秒钟的网络抖动**烧穿，症状全一样：**网络早恢复了，画面还一直转圈，
甚至播放器直接被拆掉**。而它们又都长得像「保护措施」，所以格外难查——查的时候先看这四个数在哪归零。

- **`fragLoader.hedgedLoad` 的 `maxRacers` 管的是「同时几条」，不是「一共发几次」**：
  原来 `racers` 只加不减，而**离线时 fetch 是立刻失败的**——两三次快速失败在半秒内就把额度烧光，
  此后 `race()` 每次进来直接 return，一个请求都不再发，只能干等 `skipMs`（差档 20s）到点跳片。
  网络三秒后恢复也没人重试。修法：失败即 `racers--`，重试按 500ms 一直续下去，上限交给 skipMs 兜。
  **这一条不只在断网时咬人**：源站偶发几个 403/500 同样能烧穿，表现正是「预取缓存里一堆货，
  关键那一片却永远拿不到」（有缓存 + 一直转圈）
- **lane 熔断要能自愈**（`LANE_PROBATION_MS` 30s + 网络恢复时 `reviveLanes()` 整份作废）：
  熔断永久有效时，抖动那几秒里两条 lane 都在失败，先攒够 3 次的那条被判死——**而它可能正是
  换网之后唯一走得通的那条**（出口 IP 一变，直连通不通、代理会不会 403 全都变了）。
  判死之后它再也不被取用，也就永远没机会证明自己好了
- **`hlsRetryCount` 必须由「加载成功」归零**（`noteLoadOk()` 挂在 `FRAG_BUFFERED` 上）：
  原来只在 `loadVideo` 里清一次 = 本次播放的累计值。看片一小时攒够 3 次偶发失败之后，
  **下一次任何抖动都直接落到 `recoverFromNetworkFailure`**（重新取址 → 重探通道 → `giveUp()`
  把 hls.js 整个销毁），而那时网络可能只断了两秒。中间一直在正常播放，那 3 次早就不算「连续」了
- **`navigator.onLine === false` 时三处都要让路**：① 网络错误不计数、不取址、不销毁，只写
  「网络已断开，恢复后会自动继续」并等 `online`；② 加载超时那两档闹钟（10s 取址 / 15s 报错）
  每 2s 顺延重挂——它们判的是「地址死了 / 源站取不到」，本机没网时这个前提压根不成立
  （「进电梯十几秒出来，已经报加载超时」就是这么来的）；③ `skipSegment` 直接返回 false，
  断网跳片纯属有害（下一片同样下不来，白扔一片缓存还硬跳一下画面）
- **`online` 事件要做三件事，少一件就是「网络好了画面还转圈」**（`useVideoEngine.onNetworkOnline`）：
  作废 lane 熔断记录 → `hls.startLoad()` 把停在 fatal 上的 hls.js 叫起来（**只在没在播时**，
  正播着的流被 startLoad 打断会白丢一次缓冲）→ `primePrefetch()` 立刻满上（预取失败不重排队，
  否则要等下一拍心跳）
...
- **存货厚时「播放消耗」那一项要打折**（`COAST_WALL_SECS`）：预加载时长 100s、已缓存 98s，
  ...
  → 回到全额供给，所以这条仍然压不出卡顿
- **折扣还要按「缺口快没了」淡入**（`COAST_GAP_WALL_SECS`，10s 墙钟），只看存货厚不厚会把平衡点
  **永久钉在目标值下方**：满折 0.9 时 `needRate = rate×0.1 + 缺口/60`，解「供需相等」得
  平衡点恒为 `预加载时长 − 0.9×rate×60`（1x 下 −54s），**跟用户填多少无关**。
  实测就是「预加载 300s、缓存卡在 240s 不上不下」——看着像源站慢，其实是折扣正好把供给抵成了持平。
  折扣的立论只在「缺口已经很小」时成立（当初那个例子是 100 里差 2），所以按缺口墙钟秒线性淡入：
  缺口 ≥10s 墙钟 → 不打折（照常往上补），缺口→0 → 满折。新平衡点在「目标 − 8 秒左右」，
  而且是一路缓慢爬上去的
...
| ⑤ | 缺口速率 | 距预加载时长（够播秒 × 倍速）还差多少 | 按所需速率收敛 | 只省流量，不救命 |
...
#### 超快倍速（3.5~5x）做成开关，默认关

菜单里那 10 档封顶 3x，`TURBO_PLAYBACK_RATES`（display.ts 末尾）里的 3.5/4/4.5/5 由齿轮菜单的
「超快倍速」接上去（`media.turboRate`，进 `SavedState`）。默认关的两条理由都写进了菜单的副标题——
不写在脸上的话，用户选了 5x 发现没声音又一直卡，只会当成播放器坏了：

- 带宽按倍数吃：5x 要 5 倍码率的**持续**供给，大部分源喂不动（`requiredConn` 会直接顶到 hostCap）
- **4x 往上多数浏览器直接静音**，这是浏览器的硬规则（Chrome 的音频播放上限就是 4x），不是我们能改的

两处配套：**菜单和 `</>` 快捷键共用 `controls.rateOptions`**（各写一份的话快捷键会步进到菜单里
根本没有的档位）；**关掉开关时把 `desiredRate` 收回 3x**，否则开关关了倍速还停在 5x、菜单里
又不再显示那一档，看着就是「关不掉」。`autoRateCap = max(2, desiredRate)` 不用动——
自动最佳倍速本来就跟着用户选的上限走，解锁之后它自然也能爬到 5x。
新常量放**文件末尾**：display.ts 头上那条「数组常量后面紧跟的导出会被 unimport 静默漏掉」还生效。
...
- **「预加载时长」的单位是「够播几秒」（墙钟），不是「缓存几秒视频」**：两者差一个倍速，
  3x 下要缓存 90 秒视频才等于「够播 30 秒」。换算只在**一处**做（`useHlsPrefetch.effectivePrefetchTarget`
  乘回倍速），因为它的所有比较对象（`cachedAhead`、缺口）都是视频秒。
  这么定是为了跟「存货保险线」用**同一把尺子**——两个输入框都以「够播几秒」计量，
  用户不必在脑子里做倍速换算；在此之前同一个数字在 1x 和 3x 下代表的实际余量差三倍，光看界面完全看不出来。
  **现默认 60 秒可播 / 1024MB**
- **它不再参与 hls.js 的 MSE 窗口**：单位不同（hls.js 要视频秒），喂过去就是喂错量。
  而且原来也几乎没生效过——`Math.min(30, 用户值)`，填 600 也只是 30。
  MSE 窗口现在是写死的技术天花板：`maxBufferLength: 30` / `maxMaxBufferLength: MSE_CEILING_SECS`（60），
  `hlsConfig` 与统计面板共用那个常量（同一个算式写两遍必然漂移）。
  「最大缓冲时长」输入框（`maxMaxBufferLength`）早已删掉，理由同上
- **预读深度与内存上限都不能一味给大**（两项曾都是 3600，是偶发「整个浏览器像卡死」的根因）：
  ...
  `maxBufferSizeMB` 是 LRU 天花板。抗卡真正吃紧的是「濒卡 <30s」那段，
  缓 10 分钟与缓 1 小时对流畅度差别几乎为零，内存却差 5 倍
- **改默认值救不了老用户**（`video-player-state` 优先级更高）→ `types.ts` 的 `migrateHlsTuning` 做一次性迁移，
  **只认旧默认值精确匹配**（3600 和 600 两代），不做区间钳制（用户手填的数字是明确意图，悄悄改更糟）。
  600 也要迁：按新语义读它是「够播 10 分钟」，3x 下等于缓存半小时视频，比它原本的意思还夸张
...
- **这条路上一律重解析，不再吃交接槽**。曾经的捷径是：槽里若是同一份列表（按 `pageUrl` + **线路名**比，
  光比序号会把另一条线路的列表错当成这份）就直接装上，省掉好几秒解析。已按需求删除
  （`mount()` 的 `fromSlot` 分支、`syncUrlToQuery` 里那句 `writeHandoff` 一并去掉）。
  代价：每次进播放器、每次刷新都重解析一遍，慢站几秒，nbmovie 系还可能撞限流。
  **按需取址的站点不会因此播不了**——`loadFromParseSource` 自己会 `setLazyTask` 拿到新作业单。
  集数只认 query 里的 `index`/`ep`；槽里那份 `index` 一并弃用（它本来就只在槽匹配时才敢用，
...
- **槽只剩「载荷」这一个身份，不再兼任缓存**：`?handoff=1` 和短列表的集名靠它传递，
  而「`?parseUrl=` 进来时拿槽顶替解析」那条捷径已删（见上节）。所以槽里的内容在
  `?parseUrl=` 那条路上**没人读**——将来别再顺手去读它，那份会随着切集持续过期
