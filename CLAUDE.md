...
**版式参照安卓客户端**：播放器排最前、**这一页不出站点品牌栏**（`layouts/default.vue` 的 `showHeader` 把
`/video-player` 也排除，`main` 的 `pt` 归零 → 画面贴视口顶边；返回入口做进 `TopBar` 的箭头，
全屏时它是「退出全屏」、小窗时是「离开这一页」）。**画面尺寸保持原样**：窄屏 `w-screen` 通栏贴边，
宽屏待在容器里。⚠️ 试过 `sm:w-[calc(100vw-3rem)]` 想铺更宽 —— **`calc` 里 `-` 两边必须有空格**，
无效声明被整条丢掉 → 基础的 `w-screen` 在所有断点生效，画面当场变成整个视口宽、高度顶出屏幕
（Tailwind 里要写成 `calc(100vw_-_3rem)`）。
**时间分列进度条左右两侧**（不是「07:27 / 2:07:51」挤一处）；窄屏整组内联在按钮行里，宽屏才 `order-first` 独占一行。
控制栏右侧一组：**换源 · L4（线路号）· 倍速 · 选集 · 齿轮 · 全屏**（选集从顶栏挪来了）。
**只有输入框、播放器、选集常显**；看片当下才改的进**画面内的右侧设置抽屉**（`SettingsPanel.vue`，
**不能用 UCheckbox/UInput**），排查才看的进 `CollapseCard`（里面的组件**不要自带 `UCard`**）。

**画面里的浮层一律做成抽屉/底部抽屉，不做「往上展开的气泡菜单」**：竖屏时播放器只有 200 多 px 高，
而容器是 `overflow-hidden` → 气泡会被裁掉一半，`max-h-[60vh]` 这种写法在手机上远大于播放器本身、
形同没有上限。设置与换源占右侧 70%、选集是底部抽屉，各自 `overflow-y-auto`。倍速那个小气泡留着，
但高度上限必须分两档写死（`max-h-[104px] sm:max-h-[228px]`）。**三块互斥**（都摊在右侧，同开就是叠着），
且**点画面先关浮层**（`controls.closeOverlays()` 排在切控制栏/暂停之前）。

**画面尺寸与硬件解码**（都在设置抽屉里，随 `video-player-state` 持久化）：
`fitMode` 非全屏一律保持 16:9 的盒子（页面版式不能被片源比例带着跳），只换 `object-fit`；全屏才换盒子形状。
`hwDecode` **不是真的开关硬解** —— 浏览器压根不提供那个 API（Chrome 启动参数级别的事），
关掉它的实际含义是 hls.js 的 `videoPreference: { videoCodec: 'avc1', allowedVideoRanges: ['SDR'] }`，
即**只挑 H.264/SDR 的档**：真正让弱解码器音画不同步、拖不动进度的就是 HEVC 与 HDR 那几档。
这句解释必须写在面板上，否则「关了没变化」只会被当成开关坏了。它是构造配置 → **切换必须重载视频**
（先 `saveCurrentProgress()` 再 `loadVideo()`）。旁边那枚「硬解/软解」徽标读的是
`mediaCapabilities.decodingInfo().powerEfficient`，是「这套参数能不能走硬解通路」而非运行时实测。
...
- **锁定键（未锁定态）只在全屏出**：防误触本来就是全屏握持时才需要的事，而小窗里它左侧垂直居中，
  正好压在中央播放键旁边。样式照安卓端：一枚半透明黑的圆角方块，不是渐变大圆（后者在黑画面上像警告标）
- **解锁键在锁定态下无条件渲染**（`isLocked ? showLockBtn : (isFullscreen && …)`）——它是画面上唯一的出口，
  条件里带上 `isFullscreen` 会导致「全屏锁定 → 系统退出全屏」之后**点什么都没反应，只能刷新页面**。
  同理**锁定态的 `pointerdown` 分支里必须补一发 `consumeAutoFullscreen()`**：那条路径直接 return，
  不补就没有任何时机把全屏要回来
- **「切到别的应用」的判据是 `document.hidden || !document.hasFocus()`，绝不能只听 `visibilitychange`**：
  Windows 上 alt-tab 到别的应用、点一下别的窗口，标签页**仍然是 `visible`**，
  `visibilitychange` 一声不响。只听它的话，「切应用 → 暂停 / 保住锁定 / 回来接着播」这一整套
  **在 Windows 上从来不会触发**（用户报「windows 切换应用，全屏播放锁定没有保存」）。
  所以要同时挂 `window` 的 `blur`/`focus`，并用一个 `backgrounded` 标志去重
  （两个信号在手机上常常一起来）。切走那一刻顺手 `saveCurrentProgress()` —— 这一走完全可能
  就直接关标签页了，`beforeunload` 未必来得及
- **锁定状态只由用户点解锁键解除，别处一概不许清它**。踩过两次，两次都是「锁上 → 切个应用 → 锁没了」：
  ① **`onPause` 里曾无条件 `isLocked = false`**（理由「画面停了就没什么可防的」）——
  可 `pause` 事件的来路远不止用户点暂停：抗卡会主动 pause 去攒秒数、卡死自救会动播放头，
  而**锁定态切走应用时我们自己就会 pause**，于是那一发把锁解了；
  ② **`fullscreenchange` 里曾按「是不是在后台」猜这一发是谁退的**——Windows 上
  `fullscreenchange` 常常**在窗口重新获得焦点之后**才派发，那时 `hasFocus()` 已是 true
...
- **锁定态下不猜时序，直接开看门狗**（`lockFsTick`，1s 一拍，只给触摸端、后台不跑）：上面那一整套
  （记「切走前是不是全屏」、判「这一发退全屏是谁干的」、掐「刚回前台」的窗口）**全是在猜时序**，
  而退全屏那一发落在切走前、后台里、还是回前台之后，跟机型、跟视频有没有在后台继续播都有关系
  —— 猜错一次就是「切回来变窄屏」。锁定态给了一个不用猜的判据：**锁屏本身就是
  「我在横屏看片、别动画面」的表态**，所以这个状态下压根不该存在窄屏或竖屏。于是只看当下对不对：
  掉出全屏 → 挂意图 + 每 2s 试一发 + 绑 tap；还在全屏但方向锁被释放了 → 补 `lockLandscape()`。
  **这才是这一类问题的主力**，前面那几条只是让「不锁定」的场景也尽量对
- **挂起的全屏意图要分 `restore` 和 `setting` 两种**（`pendingIsRestore`）：
  ...
  桌面上「窗口重新获得焦点」不算用户激活，`requestFullscreen` 必被拒，
  就地作废等于 Windows 上切回来永远回不到全屏。配套：**鼠标的 pointerdown 只兑现 `restore`**
  （`consumeAutoFullscreen(true)`），触摸端两种都兑现
- **「后台播放」开关（`bgPlay`，默认关，随 `video-player-state` 持久化）不能只做「我们不主动暂停」那一半**：
  安卓 Chrome 在标签页转入后台时会自己把 `<video>` 停掉（省电策略，跟自动播放策略是两回事），
  我们一个事件都收不到就已经停了 → 必须切走后补几发 `play()`（120/400/900/2000ms，那策略的生效时刻
  机型之间差好几百毫秒）。**判据只能是「切走那一刻在播」的快照，不能是 `isPlaying`**
  ——浏览器那一发暂停会派发 `pause`，`isPlaying` 当场变 false，拿它当条件等于永远抢不回来。
  窗口就这 2 秒，之后一概不管（别跟系统掰手腕）。开着时它**压过锁定态的主动暂停**。
  面板上必须写明「部分手机息屏后仍会被系统暂停」，否则只会被当成开关坏了。
  **它还会把「系统什么时候退全屏」整个时序改掉**（两次都表现成「关着一切正常、开了就出事」）：
  关着时视频在后台是停的、安卓当场退全屏，那一发落在后台好接；开着时全屏能一直挂到回前台，
  退全屏那一发**晚于** `visibilitychange` 落下来 → ① 「刚回前台」窗口 2s 接不住（放宽到 6s），
  ② 全屏若一直没被退掉，`requestFullscreen` 那条路走不到 → **`lockLandscape()` 没人再调**，
  而安卓切走应用时会释放 orientation lock → 「还在全屏但变竖屏」。所以
  **回前台时只要还在全屏就无条件补一发 `lockLandscape()`**
- **锁定态切走应用要主动暂停（没开后台播放时）、回来自动接着播**，且回来那一刻**控制栏/解锁键/三块浮层全部压下去**
  ——锁定态的语义就是「画面上别出东西」，弹一堆出来等于每次切回来都要再点一下才干净
- **自动全屏是挂起 + 补兑现**，且**只在触摸端 + 横屏生效**：桌面上会变成「点画面任何位置都被拽进全屏」；
  而**触摸端竖屏必须一票否决**（`isPortrait()`）——挂起意图唯一的兑现时机是「用户碰画面」，
  竖屏下最常见的那一碰就是点中间播放，于是「点一下播放」必然把人拽进一个上下全是黑边的竖屏全屏
  （竖屏全屏本来就只是把 16:9 钉在屏幕正中）。意图留着，转成横屏后下一次触碰再兑现
- **自动播放失败要分两类**：`NotAllowedError` 才是被策略拦了（改静音重播），其余（**主要是 `AbortError`**）
  是被新的 load 打断、重试即可——一律按「被拦」处理的表现是**自动跳到下一集却停在暂停**
- 控制栏默认收起且暂停时也收；窄屏减项且**不能 `flex-wrap`**；**倍速菜单开着时只让一次路**
  （无条件顺延会让「点开看一眼当前几倍速」把控制栏永久钉在画面上）；
