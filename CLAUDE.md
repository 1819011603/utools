...
渲染一律走 `components/ProbeMatrix.vue`——各写一遍必然漂移，尤其「四条通道全 `fail`」与
「没测过（全 `skip`）」这两种长得像却含义相反的情况（后者是「分片跟随清单通道」，报错就是误报）。

- **`fatal`（实测证伪）必须当场提醒**：典型是清单能取到、分片四条通道全 403——那一刻就注定播不了，
  可后面还要跑「重探 → 线性阶梯 5 级」，每级一次 15s 加载超时，用户盯着转圈一分多钟才见到
...
从不重新取址，于是重探/线性阶梯 5 级全程白等，用户看到的是「自动跳到下一集然后卡死在转圈上」。
