...
- **切走再回来要把全屏要回去**（安卓系统常退掉，Windows 上切窗口也会），会被拒 → 挂起-补兑现。
  **`fullscreenchange` 里 `isBackgrounded()` 为真时一律什么都不做**：那是系统替他退的，
  锁定状态和挂起意图都要原样留着；只有用户自己退出全屏才解锁。
  这里也不能用 `document.hidden` —— 它在 Windows 上恒为 false，于是「alt-tab 导致的退出全屏」
  被当成用户自己退的 → 当场解锁 + 作废意图，锁定就此丢了
