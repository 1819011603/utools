...
- **切走标签页再切回来会「卡一下」，是心跳被节流的连锁反应**（实测被问过，别当成源站问题）：
  整个预取引擎都挂在那个 1 秒心跳上，而浏览器会节流后台标签页的定时器。后台期间播放照常
  消耗预取缓存、补片却几乎停了 → 切回来时要的分片不在缓存里 → 走网络。而并发是每拍 +1 慢慢爬的，
  自己恢复要好几秒；**期间再切走切回来就好了**，因为缓存已经填回去——这个「第二次就正常」
  是该问题的特征现象。全屏时没事同理：全屏标签页始终前台，没被节流过。
  修法是 `useVideoEngine.onVisibilityChange`：回前台不等下一拍，立刻跑一拍闭环 + `primePrefetch()`
  把并发拉起来。**必须先调 `stall.resetSampler()`**，否则位置采样的 `tickAt` 还停在切走之前，
  播放头恰好没动时会把整段后台时间回填成一次假卡顿，自愈环还会据此把倍速压回 1x。
...
- `videoPlayer/useSegmentCache.ts`：模块级单例内存缓存，TTL 1 天 + 内存上限 LRU + seek 时批量 abort。
  ...
  - **预读深度与内存上限都不能一味给大**（两项曾都是 3600，是偶发「整个浏览器像卡死」的根因）：
    `maxBufferLength` 是 `effectivePrefetchTarget()` 的直接来源，3600 就是让引擎预读到一小时之后，
    1080p 3Mbps 能堆出 1GB+ 的 ArrayBuffer；`maxBufferSizeMB` 是 LRU 天花板，3600 意味着堆到
    3.6GB 才淘汰。现默认 600s / 1024MB。抗卡真正吃紧的是「濒卡 <30s」那段，
    缓 10 分钟与缓 1 小时对流畅度差别几乎为零，内存却差 5 倍。
    注意**改默认值救不了老用户**：`video-player-state` 优先级高于默认，得点「重置默认」。
  - **后台标签页是浏览器内存回收的首选对象**，所以 `visibilitychange → hidden` 时要立刻
    `purgePlayedSegments()` 把已播分片吐掉。否则几百 MB 跟着进后台，切回来整体换页，
    主线程整段阻塞——症状是「浏览器像卡死，再切一次才好」，且**只在播久了才偶现**
    （要攒够量），很容易误判成源站或网络问题。
