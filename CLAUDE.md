...
- **「用户按下暂停」豁免节流跑一轮**（`requestSyncFlush` → `syncNow({ skipThrottle: true })`）：
  那一刻进度刚落库、人多半要走开或换设备，正该把它推上去；跑完 `lastSyncAt` 更新，
  5 分钟窗口从那一刻重新开始算。**只豁免节流，不豁免「有变更才同步」**。
  **必须挂在 `togglePlay` 上，绝不能挂 `<video>` 的 `pause` 事件**——抗卡时引擎会主动 pause
  去攒够播的秒数，一卡就同步一轮是往火上浇油。另外连点播放/暂停要有 30 秒去重
