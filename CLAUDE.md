...
- **`video-watch` 同一部剧以本机那条为准**（`SyncSpec.preferLocal`）：这台设备上正看着的那部剧，
  本地那条记的就是眼前正在发生的观看，让远端按时间戳把它顶掉的话续看提示会当场跳到另一台的集数上。
  **不同的剧不受影响**——那是不同的键，照旧取并集，两边的剧都留着。
  代价要认：同一部剧变成「谁最后同步谁赢」而不是「谁看得晚谁赢」。其余几份仍按时间（`show-prefs` 按 `mt`）
