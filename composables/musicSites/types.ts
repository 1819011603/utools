...
export interface MusicSite {
  ...
  /**
   * `true` = **这个站点只在本地开发时出现**，线上从搜索界面里摘掉（见 `MUSIC_SITES`）。
   *
   * 给的是「源站拦 Cloudflare 出口」这一类站点（fangpi 就是）：从 Workers 里能改的只有请求头，
   * 出口 IP 和 TLS 指纹都不在我们手里，浏览器又因为没有 `ACAO` 抓不了 ——
   * 也就是说线上**没有能修的东西**，摆在界面上就只是个「点了必报错」的坑。
   *
   * 注意它**只影响搜索界面的站点列表，不影响 `siteById`**：存量收藏里可能躺着这个站点的曲目，
   * 查得到才能给出「这个源线上用不了」这句准话，查不到就变成含糊的「不认识这个音源」。
   */
  localOnly?: boolean
}
