...
export interface ParseRule {
  ...
  /**
   * 封面图（给「播放历史 / 收藏」当缩略图）。**只在详情页上跑**——
   * 播放页底下那一片「猜你喜欢」全是**别的片子**的封面（实测 ncat22 播放页里
   * 第一个 `/vod1/vod/cover/…` 就是隔壁那部剧），在那儿抠必然张冠李戴，
   * 而这种错在界面上完全看不出来（图是好的，只是不是这部剧）。
   *
   * 不填也没关系：绝大多数站点的 `og:image` 就是海报，通用抓取（`parseCover`）已经够用，
   * 这条是给「连详情页都不写 og:image」的站点补的。
   */
  coverRe?: string

  /**
   * 封面的可用图床（同 `SearchRule.picBase`）：有的站点页面里写的是相对路径，
   * 而那个路径在站点自己域名下恒 403（实测 ncat22 的 `/vod1/…` 就是），
   * 可用域名列在它自己引的某个 js 里 —— 现抠现用，站点换域名不必改代码。
   */
  coverBase?: {
    /** 从页面里抠出那个 js 的地址 */
    fromRe: string
   ...
   * 两者都只是**候选值**，播放器仍按 直连 → 代理·伪装 → 这对头 逐级实测降级，
